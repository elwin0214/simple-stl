/*
  平衡树，有个一个不存储实际值的节点head_,该节点left、right分别指向最小、最大节点
  parent指向树的根节点。
  旋转方式可以参考：http://blog.csdn.net/gabriel1026/article/details/6311339
*/
...
public:
  treenode_iterator(node_ptr_type head, node_ptr_type node_ptr)
    : head_(head),
      node_ptr_(node_ptr)
  {
